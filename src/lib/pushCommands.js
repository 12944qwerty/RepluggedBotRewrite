...
export async function pushCommands() {
    if (slashCommandData.length === 0) {
        console.log("[Slash Command Pusher]: No commands to push!".red);
    } else {
        try {
            console.log(`[Slash Command Pusher]: Attempting to push ${slashCommandData.length} commands to Discord.`.magenta);
            if (devMode) {
                console.log(`[Slash Command Pusher]: Pushing commands to guild: ${guildID}`.magenta);
                await restAPI.put(Routes.applicationGuildCommands(clientID, guildID), { body: slashCommandData });
            } else {
                console.log(`[Slash Command Pusher]: Pushing commands to global scope.`.magenta);
                await restAPI.put(Routes.applicationCommands(clientID), { body: slashCommandData });
            }
        } catch (error) {
            console.error(error);
        } finally {
            console.log(`[Slash Command Pusher]: Successfully pushed ${slashCommandData.length} commands to Discord.`.green);
        }
    }
}